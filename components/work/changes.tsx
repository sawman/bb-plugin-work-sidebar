...
function pullRequestStatus(pullRequest: CurrentPullRequestView): { icon: IconName; tone: string; label: string } {
  ...
  if (pullRequest.state === "draft") return { icon: "Circle", tone: "draft", label: "Draft" };
...
export function CurrentPullRequestCard({ pullRequest, expanded, onToggle }: { pullRequest: CurrentPullRequestView; expanded: boolean; onToggle(): void }) {
  ...
  const reviewSignal = pullRequest.review.state === "approved" ? { icon: "Check" as const, tone: "success", label: "Approved" } : pullRequest.review.state === "changes_requested" ? { icon: "Wrench" as const, tone: "problem", label: "Changes requested" } : pullRequest.review.state === "review_requested" ? { icon: "User" as const, tone: "pending", label: "Review requested" } : pullRequest.review.state === "review_required" ? { icon: "Eye" as const, tone: "pending", label: "Review required" } : { icon: "Circle" as const, tone: "pending", label: "No review" };
  return <article className="ws-current-pr-card"><div className="ws-current-pr-summary"><span className={`ws-pr-state-icon ws-pr-state-${status.tone}`} title={status.label} aria-label={status.label}><Icon name={status.icon} aria-hidden /></span><span><strong>#{pullRequest.number} {pullRequest.title}</strong><small>{pullRequest.head} · {readableStatus(pullRequest.review.state)} · {pullRequest.checks.passedCount}/{pullRequest.checks.totalCount} checks</small></span><span className="ws-current-pr-signals" aria-label="Pull request signals">{checkSignal && <span className={`ws-current-pr-signal ws-pr-state-${checkSignal.tone}`} title={checkSignal.label} aria-label={checkSignal.label}><Icon name={checkSignal.icon} aria-hidden /></span>}<span className={`ws-current-pr-signal ws-pr-state-${reviewSignal.tone}`} title={reviewSignal.label} aria-label={reviewSignal.label}><Icon name={reviewSignal.icon} aria-hidden /></span></span></div><a className="ws-current-pr-open ws-pr-tooltip" data-tooltip="Open on GitHub" href={pullRequest.url} target="_blank" rel="noreferrer" aria-label={`Open pull request #${pullRequest.number} on GitHub`}>↗</a><button type="button" className="ws-current-pr-expand ws-pr-tooltip" data-tooltip={expanded ? "Hide PR details" : "Show PR details"} onClick={onToggle} aria-expanded={expanded} aria-label={`${expanded ? "Hide" : "Show"} details for pull request #${pullRequest.number}`}>{expanded ? "⌄" : "›"}</button>{expanded && <div className="ws-current-pr-details"><span>Review: {readableStatus(pullRequest.review.state)}</span><span>Checks: {pullRequest.checks.passedCount} passed · {pullRequest.checks.pendingCount} pending · {pullRequest.checks.failedCount} failed</span><span>Merge: {readableStatus(pullRequest.mergeability.state)}</span></div>}</article>;
}
