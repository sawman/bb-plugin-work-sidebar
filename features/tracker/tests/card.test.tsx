...
const context = { tasksAvailable: true, currentThread: { title: "Fixture", status: "idle" as const, runtimeStatus: "idle", providerId: "codex" }, tasks: [], subtasks: [], outcome: null, executionTasks: [], bindings: [], legacy: { state: "none" as const, taskIds: [], message: null }, goal: null, todos: [], children: [] };

function fixture(overrides: Partial<Rpc> = {}): Rpc { return { getWorkContext: () => context, getChanges: () => ({ currentPullRequest: null, stack: null, stackUnavailableReason: null, githubStack: null, repository: { outcome: "absent", message: null, branch: null, base: null, ahead: 0, behind: 0, worktreeState: null, hasUncommittedChanges: false, changedFileCount: 0, changedInsertions: 0, changedDeletions: 0, changedFiles: [] } }), sidebarTasks: () => ({ available: true, tasks: [], projects: [], error: null }), sidebarTaskLinks: () => ({ available: true, links: {}, error: null }), getWorkStatus: () => ({ currentThread: context.currentThread, children: [] }), getLatestActivity: () => ({ currentThread: { status: "idle", runtimeStatus: "idle" }, latest: null, lastUser: null, current: null }), getWorkOutcome: () => ({ tasksAvailable: true, outcome: null, executionTasks: [], bindings: [] }), getWorkGoal: () => null, getWorkPlan: () => ({ items: [] }), getWorkProviderStatus: () => ({ tone: "green", providerId: "codex", providerName: "Codex", statusUrl: null, status: "ready", message: null }), getGitHubApiHealth: () => ({ state: "available", scope: "unknown", message: null, retryAt: null }), getWorkTracker: () => unlinked, searchLinearIssues: () => ({ items: [] }), ...overrides } as Rpc; }
...
afterEach(() => { cleanup(); getPluginQueryClient().clear(); vi.clearAllMocks(); vi.useRealTimers(); });

describe("registered tracker card", () => {
  it("dedupes header/card context and debounces search through actual Query hooks", async () => {
    vi.useFakeTimers(); const app = await loadPluginApp(() => import("../../../app")); const getWorkTracker = vi.fn(() => unlinked); const searchLinearIssues = vi.fn(() => ({ items: [] }));
    const slot = renderSlot(app.threadPanelActions[0]!, { threadId: "thr_tracker", params: null }, { rpc: fixture({ getWorkTracker, searchLinearIssues }) });
    await act(async () => { await vi.advanceTimersByTimeAsync(0); }); expect(getWorkTracker).toHaveBeenCalledTimes(1);
    fireEvent.change(slot.getByLabelText("Search Linear issues"), { target: { value: "LIN" } });
    await act(async () => { await vi.advanceTimersByTimeAsync(179); }); expect(searchLinearIssues).not.toHaveBeenCalled();
    await act(async () => { await vi.advanceTimersByTimeAsync(1); }); expect(searchLinearIssues).toHaveBeenCalledWith({ threadId: "thr_tracker", query: "LIN" });
    expect(slot.getByRole("listbox", { name: "Suggested Linear issues" })).toBeTruthy(); slot.lifecycle.unmount();
  });

  it("keeps Work siblings while tracker is loading, unavailable, non-Linear, or malformed", async () => {
    const app = await loadPluginApp(() => import("../../../app"));
    for (const result of [new Promise<never>(() => undefined), { ...unlinked, available: false, message: "Taskboard absent" }, { ...unlinked, visible: false }, undefined]) {
      getPluginQueryClient().clear(); const slot = renderSlot(app.threadPanelActions[0]!, { threadId: `thr_${Math.random()}`, params: null }, { rpc: fixture({ getWorkTracker: () => result as never }) });
      await waitFor(() => expect(slot.getByText("Status")).toBeTruthy()); expect(slot.getByText("Tasks")).toBeTruthy(); expect(slot.getByText("Outcome")).toBeTruthy(); slot.lifecycle.unmount();
    }
  });
  ...
  it("uses the exact link RPC and restores its disabled option after rejection", async () => {
    ...
    link.reject(new Error("link failed")); await waitFor(() => expect((slot.getByRole("option") as HTMLButtonElement).disabled).toBe(false)); expect(toast.error).toHaveBeenCalledWith("link failed"); slot.lifecycle.unmount();
  });
  ...
  it("issues exact linked status and unlink RPCs with disabled controls and targeted refreshes", async () => {
    ...
    status.resolve({ key: "LIN-1", status: "Done" }); await waitFor(() => expect((slot.getByLabelText("Linear issue status") as HTMLSelectElement).disabled).toBe(false)); expect(toast.success).toHaveBeenCalledWith("LIN-1 moved to Done");
    fireEvent.click(slot.getByRole("button", { name: "Unlink" })); await waitFor(() => expect(slot.inspection.rpcCalls.filter((call) => call.method === "unlinkLinearIssue")).toHaveLength(1)); expect(slot.inspection.rpcCalls.find((call) => call.method === "unlinkLinearIssue")?.input).toEqual({ threadId: "thr_linked" }); expect((slot.getByRole("button", { name: "Unlink" }) as HTMLButtonElement).disabled).toBe(true); unlink.reject(new Error("unlink failed")); await waitFor(() => expect((slot.getByRole("button", { name: "Unlink" }) as HTMLButtonElement).disabled).toBe(false)); expect(toast.error).toHaveBeenCalledWith("unlink failed"); slot.lifecycle.unmount();
  });
  ...
  it("uses BB navigation for both the header badge and linked issue", async () => {
    const app = await loadPluginApp(() => import("../../../app"));
    const slot = renderSlot(app.threadPanelActions[0]!, { threadId: "thr_nav", params: null }, { rpc: fixture({ getWorkTracker: () => linked }) });
    await waitFor(() => expect(slot.getByLabelText("Linear issue status")).toBeTruthy());
