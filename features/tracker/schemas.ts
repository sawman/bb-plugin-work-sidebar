...
export const trackerContextSchema = z.object({
  visible: z.boolean(), available: z.boolean(), message: z.string().nullable(),
  suggestions: z.array(z.object({ key: z.string(), title: z.string(), url: z.string().url() })),
  item: trackerItemSchema.nullable(), statusOptions: z.array(trackerStatusOptionSchema),
});
...
export const trackerRpcSchemas = {
  ...
  linkLinearIssue: { input: threadInput.extend({ key: z.string().trim().min(2).max(64) }).strict(), output: z.object({ key: z.string(), title: z.string() }) },
  searchLinearIssues: { input: threadInput.extend({ query: z.string().trim().max(160) }).strict(), output: z.object({ items: z.array(z.object({ key: z.string(), title: z.string(), url: z.string().url() })) }) },
  unlinkLinearIssue: { input: threadInput, output: z.object({ ok: z.literal(true) }).strict() },
  updateLinearIssueStatus: { input: threadInput.extend({ statusId: z.string().min(1) }).strict(), output: z.object({ key: z.string(), status: z.string() }) },
};
