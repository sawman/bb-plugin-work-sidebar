...
export function ChangesStackBranchRow({ branch, signals, expanded, checkingOut, onToggle, onCheckout }: { branch: GitHubStackBranch; signals?: StackBranchSignals; expanded: boolean; checkingOut: boolean; onToggle(): void; onCheckout(): void }) {
  ...
  return (
    <li className={`ws-stack-layer-item ${branch.isCurrent ? "ws-stack-current" : ""} ${merged ? "ws-stack-merged" : ""} ${expanded ? "ws-stack-expanded" : ""}`}>
      <div className="ws-stack-layer">
        <Status presentation={statePresentation} className="ws-stack-state-icon" />
        <button type="button" className="ws-stack-layer-toggle" onClick={onToggle} disabled={fileCount === 0} aria-expanded={expanded} aria-label={filesLabel}>
          <strong>{pr ? `#${pr.number} ` : ""}{title}</strong>
          <small>
            <span className="ws-stack-subtitle-signals">
              {presented && <><Status presentation={presented.checks} /><Status presentation={presented.review} /></>}
            </span>
            {branch.name}
            {branch.diff ? <> · <b>+{branch.diff.additions}</b> <i>−{branch.diff.deletions}</i></> : null}
          </small>
        </button>
        <span className="ws-stack-actions">
          {pr && <a className="ws-pr-tooltip" data-tooltip="Open on GitHub" href={pr.url} target="_blank" rel="noreferrer" aria-label={`Open pull request #${pr.number} on GitHub`}>↗</a>}
          <button type="button" className="ws-stack-checkout ws-pr-tooltip" data-tooltip={merged ? "Merged branch" : branch.isCurrent ? "Current branch" : `Check out ${branch.name}`} onClick={onCheckout} disabled={merged || branch.isCurrent || checkingOut} aria-label={checkingOut ? `Checking out ${branch.name}` : merged ? "Merged branch" : branch.isCurrent ? "Current branch" : `Check out ${branch.name}`}>{checkingOut ? "…" : "⇥"}</button>
        </span>
      </div>
      {expanded && branch.diff && (
        <div className="ws-stack-files">
          {branch.diff.files.map((file) => <span key={file.path}><b className={`ws-file-${file.status}`}>{file.status[0]?.toUpperCase()}</b><em>{file.path}</em><small>{file.additions !== null ? `+${file.additions}` : ""} {file.deletions !== null ? `−${file.deletions}` : ""}</small></span>)}
          {branch.diff.truncated && <small className="ws-stack-files-truncated">Only the first {branch.diff.files.length} files are shown.</small>}
        </div>
      )}
    </li>
  );
