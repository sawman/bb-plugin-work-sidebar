...
export function createWorkBindingsService(bb: BbPluginApi, tasks: TasksPluginAdapter) {
  const read = async (): Promise<WorkBindings> =>
    normalizeBindings(await bb.storage.kv.get<unknown>(WORK_BINDINGS_KEY));
  const write = (bindings: WorkBindings) => bb.storage.kv.set(WORK_BINDINGS_KEY, bindings);
  ...
  const descendants = async (parentThreadId: string, depth = 0): Promise<DescendantThread[]> => {
    if (depth >= 8) return [];
    const direct = await bb.sdk.threads.list({ parentThreadId, includeHidden: true, limit: 100 });
    const nested = await Promise.all(direct.map((child) => descendants(child.id, depth + 1)));
    return [...direct.map((thread) => ({ thread, depth: depth + 1 })), ...nested.flat()];
  };
  const summarize = (binding: OutcomeBinding | ExecutionBinding): BindingSummary => ({
    ...
    dispatchState: binding.kind === "execution" ? binding.dispatchState : "ready",
    recoveryMessage: binding.kind === "execution" ? binding.recoveryMessage : null,
  });
  const links = async (): Promise<Record<string, TaskLink[]>> => {
    const saved = await read();
    const [byId, projects] = await Promise.all([tasks.allTasksById(), tasks.projects()]);
    const names = new Map(projects.map((project) => [project.id, project.name]));
    const output: Record<string, TaskLink[]> = {};
    const add = (threadId: string, task: Task, liveStatus: TaskLink["liveStatus"], role: TaskLink["role"], mode: BindingMode | null, idempotencyKey: string | null, dispatchState: DispatchState | null) => {
      (output[threadId] ??= []).push({ task: summarizeTask(task, names.get(task.projectId) ?? "Work"), threadId, liveStatus, role, mode, idempotencyKey, dispatchState });
    };
    for (const outcome of saved.outcomes) {
      const task = byId.get(outcome.outcomeTaskId);
      if (!task) continue;
      const rows = await tasks.call("listTaskThreads", { taskId: task.id }, z.object({ taskThreads: z.array(taskThreadSchema) }));
  ...
  const projectFor = async (thread: Awaited<ReturnType<typeof rootThread>>, options: { explicitTaskProjectId?: string | null; parentOutcome?: OutcomeBinding | null; createIfMissing: boolean }) => {
    const environmentProjectId = thread.environmentId ? (await bb.sdk.environments.get({ environmentId: thread.environmentId })).projectId : null;
    ...
    const created = await tasks.call("createProject", {
      name: bbProject.name,
      prefix: projectPrefix(bbProject.name, thread.projectId, new Set(projects.map((project) => project.prefix))),
      color: "blue", folderId: null, linkedBbProjectId: thread.projectId,
    }, z.object({ project: z.object({ id: z.string() }) }));
    const verified = (await tasks.projects()).filter((project) => project.linkedBbProjectId === thread.projectId);
  ...
  const legacy = async (rootThreadId: string, projectId: string): Promise<LegacyContext> => {
    const [all, projects] = await Promise.all([tasks.listAll({ activeOnly: false, sort: "manual" }), tasks.projects()]);
    const candidates: Task[] = [];
    for (const task of all.filter((candidate) => candidate.parentTaskId === null)) {
      const rows = await tasks.call("listTaskThreads", { taskId: task.id }, z.object({ taskThreads: z.array(taskThreadSchema) }));
      if (rows.taskThreads.some((row) => row.threadId === rootThreadId)) candidates.push(task);
    }
    if (!candidates.length) return { state: "none", taskIds: [], message: null };
    if (candidates.some((task) =>
      projects.find((project) => project.id === task.projectId)?.linkedBbProjectId !== projectId
    )) {
      return {
    ...
    return { state: "adoptable", taskIds: [candidates[0].id], message: "One legacy top-level attachment can be explicitly adopted." };
  };
  const outcome = async (input: { rootThreadId: string; title: string; description: string; taskProjectId?: string | null }) => {
    if (!(await tasks.available())) throw new Error("The official BB Tasks plugin is not available");
    ...
    const saved = await read();
    const existing = saved.outcomes.find((binding) => binding.rootThreadId === root.id);
    ...
    const taskProjectId = await projectFor(root, { explicitTaskProjectId: input.taskProjectId, createIfMissing: true });
    const result = await tasks.call("createTask", {
    ...
    if (result.task.parentTaskId !== null || result.task.projectId !== taskProjectId) throw new Error("Outcome creation did not return the requested top-level Tasks task");
    await tasks.call("taskThreadsAttach", { taskId: result.task.id, threadId: root.id }, z.object({ threadId: z.string() }));
    const now = new Date().toISOString();
    const binding: OutcomeBinding = { kind: "outcome", rootThreadId: root.id, outcomeTaskId: result.task.id, taskProjectId, createdAt: now, updatedAt: now };
    await write({ ...saved, outcomes: [...saved.outcomes, binding] });
    return { task: result.task, binding };
  };
  const execution = async (input: { rootThreadId: string; title: string; description: string; idempotencyKey: string }) => {
    const root = await rootThread(input.rootThreadId);
    const saved = await read();
    const outcomeBinding = saved.outcomes.find((binding) => binding.rootThreadId === root.id);
    if (!outcomeBinding) throw new Error("Ensure an outcome context before creating an execution task");
    const all = await tasks.allTasksById();
    const existing = saved.executions.find((binding) => binding.rootThreadId === root.id && binding.idempotencyKey === input.idempotencyKey);
    if (existing) {
    ...
    if (!outcomeTask) throw new Error(`Outcome binding ${outcomeBinding.outcomeTaskId} is missing; recovery is required`);
    assertOutcomeTaskBinding(outcomeBinding, outcomeTask);
    const taskProjectId = await projectFor(root, { parentOutcome: outcomeBinding, createIfMissing: false });
    const result = await tasks.call("createTask", {
      projectId: taskProjectId,
      title: input.title,
      description: input.description,
    ...
    if (!result.ok) throw new Error(result.error.message);
    if (result.task.parentTaskId !== outcomeBinding.outcomeTaskId || result.task.projectId !== taskProjectId) throw new Error("Execution task must be a direct child in the outcome project");
    const now = new Date().toISOString();
  ...
  const owner = async (input: { rootThreadId: string; idempotencyKey: string; mode: BindingMode; prompt?: string; title?: string; visibility?: "visible" | "hidden" }) => {
    const root = await rootThread(input.rootThreadId);
    const saved = await read();
    const index = saved.executions.findIndex((binding) => binding.rootThreadId === root.id && binding.idempotencyKey === input.idempotencyKey);
    ...
    if (input.mode === "direct") {
      const pending = await save(bindExecutionOwner(current, "direct", root.id, "pending_attachment", null));
      try {
        await tasks.call("taskThreadsAttach", { taskId: pending.executionTaskId, threadId: root.id }, z.object({ threadId: z.string() }));
      } catch (error) {
        ...
        return {
          binding: await save(bindExecutionOwner(pending, "direct", root.id, "recovery_required", message)),
          spawnedThreadId: null,
        };
      }
      const ready = await save(bindExecutionOwner(pending, "direct", root.id, "ready", null));
    ...
    try {
      ...
      return {
        binding: await save(bindExecutionOwner(pendingSpawn, "delegated", null, "recovery_required", message)),
        spawnedThreadId: null,
      };
    }
    const pendingAttachment = await save(bindExecutionOwner(pendingSpawn, "delegated", spawned.id, "pending_attachment", null));
    try {
      await tasks.call("taskThreadsAttach", { taskId: pendingAttachment.executionTaskId, threadId: spawned.id }, z.object({ threadId: z.string() }));
    } catch (error) {
      ...
      return {
        binding: await save(bindExecutionOwner(pendingAttachment, "delegated", spawned.id, "recovery_required", message)),
  ...
  return { read, write, rootThread, descendants, summarize, links, legacy, outcome, execution, owner };
}
